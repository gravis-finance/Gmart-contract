...
contract GravisTradingPostV2 is OwnableUpgradeable, ERC721HolderUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getOrders() public view returns (Order[] memory orders) {
        ...
        for (uint256 i = 1; i <= orderId; i++) {
            if (ordersById[i].amount > 0 && ordersById[i].price > 0) {
                totalOrders++;
            }
        }
    ...
    function getOrdersByMaker(address _maker) public view returns (Order[] memory orders) {
        ...
        for (uint256 i = 1; i <= orderId; i++) {
            if (ordersById[i].amount > 0 && ordersById[i].price > 0 && ordersById[i].maker == _maker) {
                totalOrders++;
            }
        }
    ...
    function placeOrder(
        ...
        ordersById[orderId] = Order(orderId, _msgSender(), _nft, _tokenType, _amount, _price, _ids);

        emit OrderPlaced(orderId, _msgSender(), _nft, _tokenType, _amount, _price, _ids);
    }
    ...
    function fillOrder(
        ...
        stable.safeTransferFrom(_msgSender(), maker, applyFee(fullPrice));
    ...
    function orderExists(Order memory _order) internal pure returns (bool exists) {
        return _order.price > 0 && _order.amount > 0;
    }
